...
import { Exam12_backend } from 'declarations/Exam12_backend';
import './index.scss';
...
function App() {
  ...
  const updateIdentity = (identity) => {
    if (identity) {
      ...
      const actor = Actor.createActor(Exam12_backend, { agent: agent });
      Exam12_backend.setActor(actor); // Set the actor for Exam12_backend
    } else {
      setPrincipal(null);
      Exam12_backend.setActor(null); // Clear the actor
    }
  };
  useEffect(() => {
  ...
  const fetchStudents = async () => {
    try {
      const studentsList = await Exam12_backend.getStudents();
  ...
  const fetchCourses = async () => {
    try {
      const coursesList = await Exam12_backend.getCourses();
  ...
  const fetchStudentCourses = async (studentId) => {
    try {
      const studentCoursesList = await Exam12_backend.getStudentCourses(studentId);
  ...
  const handleAddStudent = async (event) => {
    ...
    try {
      await Exam12_backend.addStudent(newStudent.firstName, newStudent.lastName, newStudent.school);
  ...
  const handleAddCourse = async (event) => {
    ...
    try {
      await Exam12_backend.addCourse(newCourse.courseId, newCourse.courseName);
  ...
  const handleAssignCourseToStudent = async () => {
    ...
    try {
      await Exam12_backend.assignCourseToStudent(selectedStudentId, selectedCourseId);
